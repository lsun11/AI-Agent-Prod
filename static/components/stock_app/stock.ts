...
export class StockGadget {
    ...
    private updateUI() {
        ...
        const diff = (data as any).realChange || 0;
        const pct = (data as any).realPercent || 0;
        const sign = diff >= 0 ? "+" : "";
    ...
    private async updateStock(ticker: string) {
        try {
            ...
            if (stockObj) {
                ...
                (stockObj as any).realChange = apiData.change;
                (stockObj as any).realPercent = apiData.percent;
    ...
    private async fetchHistory(ticker: string, range: string) {
