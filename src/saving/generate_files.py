...
def write_pdf(path: str | Path, layout: DocumentLayout, flowchart_png_path: Optional[str] = None, ) -> None:
    """
    Use your existing ReportLab logic here, but:
      - use layout.title for the title
      - use layout.report_markdown as the main content
    """
    build_pdf_document(layout.title, layout.report_markdown, str(path), flowchart_png_path=flowchart_png_path)
...
def generate_all_files_for_layout(
    ...
    write_pdf(pdf_path, layout, flowchart_png_path)
    write_docx(docx_path, layout)
    write_slides(pptx_path, layout, flowchart_png_path)
