...
# - Focus on concrete tools, platforms, or services (not vague concepts).
# - Prefer actionable insights: how someone would actually use each resource.
...
# - Keep responses concise and structured so they can be rendered in a UI.
# """
#
...
#     """
#     Base prompts for career-related research.
#     Subtopics (resume_tools, job_search, etc.) will override wording
#     but preserve the same attributes/method names.
#     """
#
#     TOOL_EXTRACTION_SYSTEM = f"""You are a career research assistant.
# Extract specific platforms, tools, or services from articles that are relevant to career development.
#
# {CAREER_PROMPT_GUIDELINES}
# """
#
...
# that could help with this career query.
#
# Rules:
# - Only include actual product/platform names, not generic concepts.
# - Focus on resources people can directly sign up for, install, or use.
...
# """
#
#     TOOL_ANALYSIS_SYSTEM = f"""You are analyzing career-related platforms and tools.
# You must structure information so it is easy to compare across options.
#
# {CAREER_PROMPT_GUIDELINES}
# """
#
#     def tool_analysis_user(self, company_name: str, content: str) -> str:
#         return f"""Platform/Tool: {company_name}
# Website Content (partial):
# {content[:2500]}
#
# Analyze this platform from a career perspective and provide:
# - pricing_model: One of "Free", "Freemium", "Paid", "Enterprise", or "Unknown".
# - pricing_details: Short string with any concrete pricing info if available
#   (e.g. "from $20/month", "Free tier + Pro $10/user/month"), or null/empty if unclear.
# - description: Brief 1-sentence description focusing on what this platform does for careers.
# - tech_stack: List of notable technologies or channels (e.g. "Web", "Mobile app", "Chrome extension").
# - api_available: true if they provide an API, SDK, or programmatic access; false if clearly none; null if unclear.
...
# - seniority_focus: Short string summarizing which seniority levels it focuses on (e.g. "Students and entry-level", "All levels").
#
# Return a valid JSON object only.
# """
#
#     RECOMMENDATIONS_SYSTEM = f"""You are a senior career coach giving concise, practical recommendations.
# You will be given structured data about multiple platforms/tools and a user's career query.
#
# {CAREER_PROMPT_GUIDELINES}
#
# Keep your recommendations short and actionable (max 3-4 sentences).
# """
#
#     def recommendations_user(self, query: str, company_data: str) -> str:
#         return f"""Career Query: {query}
# Tools/Platforms Analyzed (JSON array):
# {company_data}
#
# Provide a brief recommendation (3-4 sentences max) covering:
# - Which 1-2 tools seem best suited and why.
# - Any important pricing or paywall considerations.
# - Which roles/seniority levels each tool seems most appropriate for.
#
# Be direct and concrete. No long paragraphs.
# """
